...
module.exports = function readAssets(assetsFilename: string): Promise<Object> {
  return new Promise((resolve, reject) => {
    if (process.env.NODE_ENV === 'production' && cache) {
      resolve(cache);
    } else {
      fs.readFile(assetsFilename, (error, assetsBuffer) => {
        if (error) {
          reject(
            `Failed to read ${assetsFilename} ${error.code
              ? `(${error.code})`
