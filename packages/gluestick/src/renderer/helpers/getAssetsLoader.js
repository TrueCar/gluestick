...
module.exports = function getAssetsLoader(
  ...
  const { host, protocol, ports } = config.GSConfig;
  const hostAddress =
    process.env.NODE_ENV === 'production'
      ? ''
      : `${protocol}://${host}:${ports.server}`;
  return `
    ...
    function loadVendorThenEntry() {
      var entries = ['${vendorBundle}', '${entryPointBundle}'];
      if (isPolyfillNeeded()) {
        entries.unshift('${hostAddress}/polyfill.min.js');
      }
      loadjs(entries, {
