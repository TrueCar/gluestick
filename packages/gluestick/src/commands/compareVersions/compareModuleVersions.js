 ...
 */
const compareModuleVersions = (projectPackage, modulePath) => {
  const discrepancies = [];
  let devDepDifference = false;
  let allDependencies = projectPackage;

  if (projectPackage.dependencies === undefined || projectPackage.devDependencies === undefined) {
    if (projectPackage.dependencies === undefined || projectPackage.devDependencies === undefined) {
    ...
    allDependencies = Object.assign(projectPackage.dependencies, projectPackage.devDependencies);
  ...
  Object.keys(allDependencies).forEach(dep => {
    ...
    let tempPackage = true;
    try {
      tempPackage = require(tempPath);
    } catch (e) {
      console.log(`${chalk.red('The node_module ')}`, `${chalk.yellow(dep)}`, `${chalk.red(' does not exist')}`);
    ...
    if (!tempPackage || !semver.satisfies(tempPackage.version, allDependencies[dep])) {
      ...
      tempPackage._from.search(fileVersion) === -1)) {
        console.log(`${chalk.red('The node_module ')}`, `${chalk.yellow(dep)}`, `${chalk.red(' does not satisfy the required version in your package.json')}`);
